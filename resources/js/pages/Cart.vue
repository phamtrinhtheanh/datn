...
const page = usePage();
const products = ref(props.products); // default từ server nếu có

// Nếu chưa đăng nhập và không có props từ server
onMounted(() => {
    if (!page.props.auth.user && !props.products.length) {
        const guestCart = JSON.parse(localStorage.getItem('guest_cart') || '[]');
        products.value = guestCart.map((item: any) => ({
            id: item.product_id,
